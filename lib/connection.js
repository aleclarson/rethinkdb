...
function Connection(r, options, resolve, reject) {
  ...
  self.connection.once('error', function(connectError) {
    var error = new Err.ReqlDriverError('Failed to connect to ' + self.host + ':' + self.port);
    error.originalError = connectError;
    reject(error.setOperational());
  });
...
Connection.prototype._compareDigest = function(messageServer, resolve, reject) {
  ...
  if (self.pingInterval > 0) {
    self.pingIntervalId = setInterval(function() {
      ...
      self.r.error(PING_VALUE).run(self).fail(function(error) {
