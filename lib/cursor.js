  ...
  if (self._closed === true) {
    ...
    if ((self._data.length > 0) && (self._data[0].length > self._index)) {
      ...
      if (result instanceof Error) {
        return Promise.reject(result).notify(callback);
      }
      else {
        ...
        return Promise(result).notify(callback);
      }
    }
    else {
    ...
    if (keepGoing === false) {
      ...
      if (self._closed === false) {
        if (self._stackSize <= MAX_CALL_STACK) {
          self._next().then(resolve).fail(function(error) {
    ...
    return self._next().then(function(row) {
      if (self._stackSize <= MAX_CALL_STACK) {
        if (callback.length <= 1) {
          return Promise(callback(row)).then(nextCb)
        }
        else {
          return Promise.defer(function(resolve, reject) {
        ...
        return Promise.defer(function(resolve, reject) {
          setTimeout(function() {
            self._stackSize = 0;
            if (callback.length <= 1) {
              return Promise(callback(row)).then(resolve).fail(reject);
            }
            else {
              return Promise.defer(function(resolve, reject) {
                callback(row, resolve)
