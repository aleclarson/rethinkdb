...
var WritableStream = require('./writable_stream');
...
Term.prototype.insert = function(documents, options) {
  ...
  self._validateFields(documents);
  var term = new Term(self._r);
...
Term.prototype.update = function(newValue, options) {
  ...
  self._validateFields(newValue, true);
  var term = new Term(self._r);
...
Term.prototype._getTable = function() {
  var array = this._query;
  while (array[0] !== termTypes.TABLE) {
    if (array[0] === termTypes.VAR) break;
    array = array[1][0];
  }
  if (array[0] === termTypes.TABLE) {
    return array[1][0];
  }
  return null;
}

Term.prototype._validateFields = (function() {

  function validateDocument(document, model, partial) {
    document = model.initialize([document]);
    var error = model.validate(document, partial);
    if (error) throw error;
  }

  return function(documents, partial) {
    var models = this._r._models;
    ...
    if (Array.isArray(documents)) {
      documents.forEach(function(document) {
        validateDocument(document, model, partial);
      });
    } else {
      validateDocument(documents, model, partial);
    }
  }
})()
