...
Term.prototype._validateFields = function(documents) {
  var self = this;
  var models = self._r._models;
  if (models == null) return;

  var tableIndex = self._query.indexOf(termTypes.TABLE);
  var table = self._query[tableIndex + 1][0];
  var model = models[table];
  if (Array.isArray(documents)) {
    documents.forEach(function(document) {
      document = model.initialize([document]);
      var error = model.validate(document);
      if (error) throw error;
    });
  } else {
    documents = model.initialize([documents]);
    var error = model.validate(documents);
    if (error) throw error;
  }
}
