...
PoolMaster.prototype.fetchServers = function(useSeeds) {
  ...
  promise.then(function(feed) {
    ...
    feed.each(function(error, change) {
      if (error) {
        self._log('debug', 'The changefeed on `server_status` returned an error: ' + error.message);
        self._log.error(error);
        ...
        if (change === SEPARATOR) {
          ...
          setTimeout(function() {
            self._r.db('rethinkdb').table('server_status').run({cursor: false}).then(function(servers) {
              ...
              self._log('debug', 'Failed to fetch another copy of `server_status`: ' + error.message);
              self._log.error(error);
              //TODO Retry
            });
          }, 1000);
    ...
    self._log('debug', 'Failed to get data from `server_status` feed: ' + error.message);
    self._log.error(error);
...
PoolMaster.prototype.initPool = function(pool) {
  ...
  pool.on('new-connection', function() {
    if (self._line.getLength() > 0) {
      ...
      this.getConnection().then(p.resolve).fail(p.reject);
      self.emit('queueing', self._line.getLength())
    }
  });
